...
function App() {
  ...
  return (
    <div className={`game3d-shell ${photoMode ? 'photo-mode' : ''}${hudCompact ? ' hud-compact' : ''}`} data-hud-compact={hudCompact ? 'true' : 'false'} data-photo-mode={photoMode ? 'true' : 'false'}>
      ...
      {showDayPlan && <ModalBackdrop onClose={() => setShowDayPlan(false)}><div className="game-modal day-plan-modal day-plan-cliffhanger" data-forecast-tomorrow={forecastFor(game.day + 1).eventDay === game.day + 1 ? (EVENTS.find((e) => e.id === forecastFor(game.day + 1).eventId)?.title || forecastFor(game.day + 1).tide) : forecastFor(game.day + 1).tide} data-contract-days-left={contractProgress(game) ? String(contractProgress(game).daysLeft) : ''}><button className="modal-close" aria-label="กลับไปทำงาน" onClick={() => setShowDayPlan(false)}>×</button><small>ONE MORE DAWN</small><h2>พักทีม แล้วพบกันพรุ่งนี้</h2><p>ทีมวันนี้ยังทำได้อีก {crewLeft(game)} งาน · ต้นไม้จะเติบโตอีก 1 วัน</p><div className="cliffhanger-card" role="status"><div className="next-tide-preview" data-forecast-tomorrow><b>{forecastFor(game.day + 1).eventDay === game.day + 1 ? `พรุ่งนี้: ${EVENTS.find((e) => e.id === forecastFor(game.day + 1).eventId)?.title || 'เหตุการณ์ชายฝั่ง'}` : `พรุ่งนี้: น้ำ${forecastFor(game.day + 1).tide}`}</b><span>{habitat.living} ต้นที่กำลังเติบโต · {game.plots.filter((p) => p.species && !p.dead && p.health < 50).length} ต้นสุขภาพต่ำกว่า 50%</span></div>{contractProgress(game) ? <div className="contract-days-left" data-contract-days-left={contractProgress(game).daysLeft}><em>สัญญาฟื้นฟู</em><strong>อีก {contractProgress(game).daysLeft} วันส่งงาน</strong><i style={{ width: `${Math.max(8, (1 - (contractProgress(game).daysLeft - 1) / 2) * 100)}%` }} /></div> : <p className="mali-dawn-tip" data-character-speech="mali">มะลิ: พรุ่งนี้มีงานรอที่อ่าว — มาต่อกันอีกวันนะ</p>}</div>{contractProgress(game)?.deadline === game.day && <p className="deadline-warning">งานฟื้นฟูครบกำหนดวันนี้{contractProgress(game).ready ? ' · ส่งมอบก่อนจบวันเพื่อรับรางวัล' : ' · ถ้าจบวันจะหมดเวลาและรีเซ็ตโบนัสต่อเนื่อง'}</p>}<button className="primary-game-button large" onClick={nextDay}>ยืนยันจบวัน → วันที่ {game.day + 1}</button><button className="secondary-game-button" onClick={() => { setShowDayPlan(false); setShowRestoration(true) }}>กลับไปวางแผน</button></div></ModalBackdrop>}
